...
- **Вложения:** фотографии (сжимаются до 1024 px по умолчанию, настраивается на вкладке «Общие»; лимит файла — 16 МБ) и текстовые файлы (до 100 000 символов каждый, до 400 000 суммарно).
...
5. Откройте вкладку **FlowSlice AI**. Если вкладка не появилась — перезапустите Orca Slicer.
...
| **Общие** | Заметки для контекста, общие температура / максимум токенов / режим мышления по умолчанию; блок **сжатия контекста** (`compact_enabled`, порог в процентах, размер окна, число фото из истории); блок **фото** (сторона сжатия в px, качество JPEG, лимит файла в МБ); автообновление провайдеров (`auto_sync_providers`); сброс моделей. |
...
- **Фотографии:** можно прикрепить снимок дефекта печати или детали. Изображение сжимается до
  заданной стороны (по умолчанию 1024 px, JPEG 85 %; настраивается на вкладке «Общие», значение 0 —
  отправить оригинал без сжатия) и передаётся только моделям с поддержкой изображений. Клиентский
  лимит файла — 16 МБ, серверный — 12 МБ base64. Если выбранная модель их не поддерживает, плагин
  сообщит об этом и не станет отправлять файл.
...
- **Количество:** до 10 вложений в сообщении, из них до 10 изображений в одном запросе; суммарный
  размер изображений в запросе — до 32 МБ base64.
...
| Изображений в запросе | до 10; суммарно до 32 МБ base64 |
| Размер одного изображения | по умолчанию 1024 px, JPEG 85 % (настраивается на вкладке «Общие»; 0 — без сжатия); клиент — 16 МБ, сервер — 12 МБ base64 |
...
- Прикреплённые вами фотографии (сжимаются до заданной стороны, по умолчанию 1024 px) и текстовые файлы.
