...
- Настройки на 4 вкладках: «Модели», «Персональные», «Общие значения», «Оформление»
  - Провайдеры: DeepSeek, OpenRouter, Google, Anthropic, OpenAI, Groq, GLM, Cerebras, Mistral, xAI
  - «Персональные» — свои OpenAI/Anthropic-совместимые провайдеры и модели
  - Черновик настроек модели сохраняется отдельно — применяется только по «Сохранить»
  - Сброс настроек — только текущей вкладки; отдельные «Сброс моделей» и «Сброс персональных моделей»
...
API-ключ указывается в настройках плагина (окно чата → «Настройки» → «Модели»).
Встроенные провайдеры — DeepSeek, OpenRouter, Google, Anthropic, OpenAI, Groq, GLM, Cerebras,
Mistral, xAI. Свои OpenAI/Anthropic-совместимые провайдеры добавляются на вкладке
«Персональные».
...
- Сборка wheel: `python -m build --wheel` + `python tools/patch_wheel.py dist/*.whl`
  (имя плагина `FlowSlice AI` и автор `FlowHack` в METADATA; в wheel попадают только модули пакета).
