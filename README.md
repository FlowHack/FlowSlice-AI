...
- Остановка генерации; прерванный ответ помечается как «прервана пользователем», а не остаётся пустым
- Повторная генерация (regenerate)
- Редактирование и повторная отправка (edit & resend)
- Регенерация любого ответа, варианты ответов
...
- Синхронизация каталога моделей: кнопка «Обновить от провайдера» и автообновление
...
- Точный usage и стоимость: `stream_options.include_usage` с авто-фолбэком при HTTP 400; у OpenRouter реальные prompt/completion_tokens и cost, у Anthropic точные токены, иначе оценка. Стоимость показывается, только когда известны обе цены
...
<p align="center">
  ...
  <br><sub>Настройки, вкладка «Модели»: провайдеры, избранное, цены и зрение</sub>
</p>

<p align="center">
  <img src="assets/screenshot_general.png" width="720" alt="Настройки, вкладка «Общие»">
  <br><sub>Настройки, вкладка «Общие»: тема, язык, сжатие контекста</sub>
</p>
...
| numpy | необязательно (extra `deep`) — включает глубокий анализ геометрии модели; без него плагин работает |
...
2. После подписки плагин появится в Orca Slicer среди доступных плагинов (раздел **File → Plugins**) — установите и включите его.
3. Откройте вкладку **FlowSlice AI**. Если вкладка не появилась — перезапустите Orca Slicer.
...
В панели чата есть чекбоксы **Модель**, **Пластик**, **Принтер**, **Настройки печати** и **История**.
Отметьте то, что ассистент должен учитывать. Рядом с каждым профилем есть выпадающий список:
...
| --- | --- |
| **Модели** | Провайдер, API-ключ, модель, температура, максимум токенов, режим мышления, цена входа/выхода. Кнопка **Обновить от провайдера** и корзина — рядом. |
...
| **Общие значения** | Заметки для контекста, общие температура / максимум токенов / режим мышления по умолчанию; автообновление провайдеров (`auto_sync_providers`); сброс моделей. |
...
- Значения модели можно настраивать индивидуально; если оставить поле пустым — наследуются общие
  ...
  режиме мышления лимит токенов поднимается минимум до `16384`, чтобы модели хватило места на
  рассуждения.
- Цены входа и выхода используются для показа стоимости ответа; стоимость появляется, только когда
  известны обе цены. Токены и стоимость берутся у провайдера, когда он их отдаёт, иначе оцениваются.
...
- Кнопка **Обновить от провайдера** есть у каждого провайдера. Она подгружает полный список
  моделей провайдера в сессионный кэш — конфиг при этом не раздувается. Обновляются названия,
  цены и признак поддержки изображений.
- Чекбокс **Автообновление провайдеров** (`auto_sync_providers`) включён по умолчанию и
  срабатывает при запуске плагина и после сохранения API-ключа.
- **Цены** берутся у конкретного провайдера: у OpenRouter — собственный pricing из его каталога,
  у NordRouter — публичный прайс-лист.
- **Поддержка изображений** определяется по каталогу OpenRouter с учётом префикса провайдера
  (для всех провайдеров, кроме самого OpenRouter).
...
| --- | --- |
| **Модель** | Габариты, объём, площадь поверхности, число треугольников, положение на столе, целостность меша (manifold), количество экземпляров. |
...
из них 753 переведены на русский.
...
- **Количество:** до 10 вложений в сообщении, из них до 10 изображений в одном запросе.
...
| Команда | Описание |
| --- | --- |
| `/context` | Показать реальный список сообщений, который уйдёт модели (полный системный промпт, история, изображения кратко), и реальный подсчёт токенов этого запроса |
...
Нажмите **Обновить от провайдера** рядом с провайдером или включите **Автообновление
провайдеров** на вкладке «Общие значения» — каталог, цены и признак поддержки изображений
