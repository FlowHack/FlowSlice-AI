...
| --- | --- |
| **Модели** | Провайдер, API-ключ, модель, температура, максимум токенов, режим мышления. Кнопка **Обновить от провайдера** и корзина — рядом. |
...
- Значения модели можно настраивать индивидуально; если оставить поле пустым — наследуются общие
  (по умолчанию `temperature = 0.7`, `max_tokens = 4096`, режим мышления выключен).
...
| `/printer` | Показать сводку профилей печати |
