(function () {
  ...
  var I18N = {
    en: {
      ...
      "ctx.panel_label": "Add to chat context:",
      "ctx.panel_help": "Choose which slicer data and chat history are sent with the request. The counter next to the message field shows the estimated size of the resulting context.",
      ...
      "settings.key_set": "Key is set — enter a new one",
      ...
      "msg.model_used": "Model: {name}",
      "msg.model_title": "This reply was generated by this model",
      ...
      "settings.help_language": "Plugin UI language. The model also replies in the selected language.",
      ...
      "mp.favorites": "Favorites",
      "mp.favorite_add": "Add to favorites",
      "mp.favorite_remove": "Remove from favorites",
    ...
    ru: {
      ...
      "ctx.panel_label": "Добавить в контекст чата:",
      "ctx.panel_help": "Выберите, какие данные слайсера и историю чата отправлять с запросом. Счётчик у поля ввода показывает примерный размер итогового контекста.",
      ...
      "settings.key_set": "Ключ задан — введите новый",
      ...
      "msg.model_used": "Модель: {name}",
      "msg.model_title": "Ответ сгенерирован этой моделью",
      ...
      "settings.help_language": "Язык интерфейса плагина. На выбранном языке будет общаться модель.",
      ...
      "mp.selected": "Выбрано",
      "mp.favorites": "Избранное",
      "mp.favorite_add": "Добавить в избранное",
      "mp.favorite_remove": "Убрать из избранного",
    ...
    sr: {
      ...
      "ctx.panel_label": "Dodaj u kontekst razgovora:",
      "ctx.panel_help": "Izaberite koje podatke slajsera i istoriju razgovora poslati uz zahtev. Brojač pored polja za unos prikazuje približnu veličinu konteksta.",
      ...
      "settings.key_set": "Ključ je postavljen — unesite novi",
      ...
      "msg.model_used": "Model: {name}",
      "msg.model_title": "Ovaj odgovor je generisala ova modela",
      ...
      "settings.help_language": "Jezik interfejsa dodatka. Model će odgovarati na izabranom jeziku.",
      ...
      "mp.favorites": "Omiljeno",
      "mp.favorite_add": "Dodaj u omiljeno",
      "mp.favorite_remove": "Ukloni iz omiljenog",
  ...
  // Строит строку модели для списка выбора. Используется как в группе
  // провайдера, так и в закреплённой группе «Избранное».
  function buildModelRow(provider, item, isActive, isFav) {
    var s = state.settings || {};
    ...
    var fav = el("button", "mp-fav" + (isFav ? " active" : ""), isFav ? "♥" : "♡");
    fav.type = "button";
    fav.title = t(isFav ? "mp.favorite_remove" : "mp.favorite_add");
    (function (pid, mid) {
      fav.addEventListener("click", function (e) {
        e.stopPropagation();
        post({ type: "toggle_favorite", provider: pid, model: mid });
      });
    })(provider.id, item.id);
    row.appendChild(fav);
  ...
  function renderModelPicker() {
    ...
    var favorites = Array.isArray(s.favorites) ? s.favorites : [];
    var withModelsById = {};
    for (var w = 0; w < withModels.length; w++) {
      withModelsById[withModels[w].id] = withModels[w];
    }
    var favItems = [];
    for (var f = 0; f < favorites.length; f++) {
      var favRef = parseModelRef(favorites[f]);
      if (!favRef) {
        continue;
      }
      var favProvider = withModelsById[favRef.provider];
      if (!favProvider) {
        continue;
      }
      var favModels = mergedModels(favProvider);
      var favModel = null;
      for (var fm = 0; fm < favModels.length; fm++) {
        if (favModels[fm].id === favRef.model) {
          favModel = favModels[fm];
          break;
        }
      }
      if (!favModel) {
      ...
      var favModelName = favModel.name || favModel.id;
      var favProvName = favProvider.name || favProvider.id;
      if (query && !matchesModelQuery(favModelName, query) && !matchesModelQuery(favProvName, query)) {
        continue;
      }
      favItems.push({ provider: favProvider, model: favModel });
    }
    if (favItems.length > 0) {
      list.appendChild(el("div", "mp-group-title", t("mp.favorites")));
      for (var fi = 0; fi < favItems.length; fi++) {
        var favEntry = favItems[fi];
        var favActive =
          favEntry.provider.id === activeProvider && favEntry.model.id === activeModel;
        list.appendChild(buildModelRow(favEntry.provider, favEntry.model, favActive, true));
        shown++;
      }
    }
    for (var p = 0; p < withModels.length; p++) {
      ...
      for (var k = 0; k < groupItems.length; k++) {
        ...
        var isFav = favorites.indexOf(favoriteRef(provider.id, item.id)) !== -1;
        list.appendChild(buildModelRow(provider, item, isActive, isFav));
        shown++;
      }
    }
    if (shown === 0) {
  ...
  function renderMessage(msg, index, msgs) {
    ...
    if (msg.role === "assistant" && (msg.model_name || msg.model)) {
      var usedName = msg.model_name || msg.model;
      var modelEl = el("span", "msg-model", t("msg.model_used", { name: usedName }));
      var modelTitle = t("msg.model_title");
      if (msg.provider) {
        modelTitle = msg.provider + " / " + (msg.model || usedName) + " — " + modelTitle;
      }
      modelEl.title = modelTitle;
      timeRow.appendChild(modelEl);
    }
  ...
  // Ссылка на модель в виде "provider::model" для хранения в избранном.
  function favoriteRef(providerId, modelId) {
    return String(providerId) + "::" + String(modelId);
  }

  // Разбирает ссылку "provider::model"; для некорректной возвращает null.
  function parseModelRef(ref) {
    ...
    var provider = ref.slice(0, sep).trim();
    var model = ref.slice(sep + 2).trim();
    if (!provider || !model) {
      return null;
    }
